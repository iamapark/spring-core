...
import jyp.beans.factory.support.AbstractBeanDefinition;
    ...
    protected void loadBeanDefinition(Element ele) {
        ...
        if (id == null || "".equals(id)) {
            if (beanDefinition instanceof RootBeanDefinition) {
                ...
                logger.debug(
                    "Neither XML 'id' nor 'name' specified - using bean class name [" + id + "] as ID");
            } else {
        ...
        try {
            ...
            if (className != null) {
                ...
                if (ele.hasAttribute(DEPENDS_ON_ATTRIBUTE)) {
                    ...
                    rbd.setDependsOn(
                        StringUtils.tokenizeToStringArray(dependsOn, BEAN_NAME_DELIMITERS, true, true));
                }
        ...
        if (nl.getLength() != 1 || !(nl.item(0) instanceof Text)) {
            throw new BeanDefinitionStoreException(this.resource, beanName,
                "Unexpected element or type mismatch: expected single node of " +
                    nl.item(0).getClass() + " to be of type Text: " + "found " + ele,
                null);
        }
